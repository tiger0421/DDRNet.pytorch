...
class Tu_indoor(BaseDataset):
    def __init__(self,
        ...
        super(Tu_indoor, self).__init__(ignore_label, base_size,
        ...
        for s in range(1, num_classes):
            self.label_mapping[s] = s
        self.class_weights = torch.FloatTensor([
                                        0.004226, 0.00119, 0.241739, 0.269962,
                                        0.00058, 0.009693, 0.147234, 0.325375,
                                        ]).cuda()
        # Class_name : (B, G, R)
        self.color_map = {
            "Void"           : (0, 0, 0      ),
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = cv2.imread(os.path.join(self.root,'Tu_indoor',item["img"]),
                           cv2.IMREAD_COLOR)
        size = image.shape
        ...
        label_rgb = cv2.imread(os.path.join(self.root,'Tu_indoor',item["label"]),
                           cv2.IMREAD_COLOR)
